...
export function versionFromTag(tag) {
  if (
    ...
    throw new Error("Release tag must be semantic version.");
  return tag.replace(/^v/, "");
}

/** Validate fields binding candidate work to protected default-branch history. */
export function assertReleaseEvent({
  defaultBranch,
  eventSha,
  prerelease,
  tag,
  ...
  if (!/^[0-9a-f]{40}$/i.test(eventSha))
    throw new Error("Release event SHA must be a full Git object ID.");
  if (target !== defaultBranch)
    throw new Error("Release must target the default branch.");
  const version = versionFromTag(tag);
  if (Boolean(prerelease) !== version.includes("-"))
    throw new Error("Release prerelease state disagrees with its tag.");
  return version;
}
...
export function expectedMetadata(packageJson, packageLock, tag) {
  ...
  if (!expectedLock.packages?.[""])
    throw new Error("Package lock lacks root package metadata.");
...
export function assertCandidatePaths(paths) {
  ...
  if (
    ...
    throw new Error("Release candidate changes paths beyond package metadata.");
}
...
export async function validateNpmTarball(tarball, trustedPackage) {
  ...
  if (!stat.isFile() || stat.size > MAX_TGZ_BYTES)
    throw new Error("Tarball is not a bounded regular file.");
  const chunks = [];
  let compressed = 0;
  let expanded = 0;
  await new Promise((resolve, reject) => {
    ...
    source
      .on("data", (chunk) => {
        ...
        if (compressed > MAX_TGZ_BYTES)
          source.destroy(new Error("Tarball exceeds size limits."));
      })
      .on("error", reject);
    gunzip
      .on("data", (chunk) => {
        ...
        if (expanded > MAX_EXPANDED_BYTES)
          gunzip.destroy(new Error("Tarball exceeds size limits."));
  ...
  if (compressed > MAX_TGZ_BYTES || bytes.length > MAX_EXPANDED_BYTES)
    throw new Error("Tarball exceeds size limits.");
  ...
  for (let offset = 0; offset + 512 <= bytes.length;) {
    ...
    if (header.every((byte) => byte === 0)) {
      if (!bytes.subarray(offset).every((byte) => byte === 0))
        throw new Error("Tarball has trailing data.");
      ended = true;
      break;
    }
    ...
    if (
      ...
      throw new Error("Tarball contains an unsafe entry.");
  ...
  if (!equalJson(metadata, trustedPackage))
    throw new Error(
      "Tarball package metadata differs from the trusted candidate.",
    );
  if (!equalJson([...entries.keys()].sort(), EXPECTED_TARBALL_ENTRIES))
    throw new Error(
      "Tarball runtime entries do not match the trusted release contract.",
    );
  if (!equalJson(trustedPackage.files, ["dist"]))
    throw new Error(
      "Trusted package does not declare the expected runtime files.",
    );
  const bin =
    typeof trustedPackage.bin === "string"
      ? { [trustedPackage.name]: trustedPackage.bin }
      : trustedPackage.bin;
  if (
    !bin ||
    typeof bin !== "object" ||
    !Object.values(bin).every(
      (target) =>
        ...
        /^dist\/[A-Za-z0-9_-]+\.js$/.test(target) &&
        entries.has(`package/${target}`),
    )
  )
    throw new Error("Tarball does not contain its declared executable.");
  return {
    ...hashes(readFileSync(tarball)),
...
export async function createCandidateManifest({
  ...
  if (
    ![sourceSha, sourceTree, tagObject].every((value) =>
      /^[0-9a-f]{40}$/i.test(value),
    )
  )
    throw new Error("Candidate provenance must use full Git object IDs.");
  const version = versionFromTag(tag);
  ...
  return {
    ...
    packageJson,
    packageLock,
    artifact: { file: basename(tarball), ...artifact },
  };
}
...
export async function readCandidateArtifact(directory) {
  ...
  if (
    names.length !== 2 ||
    names[0] !== "candidate.json" ||
    names[1] !== "candidate.tgz"
  )
    throw new Error("Candidate artifact has unexpected files.");
  for (const name of names) {
    const stat = lstatSync(join(directory, name));
    if (!stat.isFile() || stat.isSymbolicLink())
      throw new Error("Candidate artifact contains a non-regular file.");
    if (name === "candidate.json" && stat.size > MAX_MANIFEST_BYTES)
      throw new Error("Candidate artifact manifest is too large.");
  }
  ...
  if (
    ...
    manifest.artifact?.file !== "candidate.tgz"
  )
    throw new Error("Candidate artifact manifest is invalid.");
  const version = versionFromTag(manifest.tag);
  if (
    manifest.version !== version ||
    ![manifest.sourceSha, manifest.sourceTree, manifest.tagObject].every(
      (value) => /^[0-9a-f]{40}$/i.test(value),
    )
  )
    throw new Error("Candidate artifact provenance is invalid.");
  const artifact = await validateNpmTarball(
    join(directory, "candidate.tgz"),
    manifest.packageJson,
  );
  for (const key of ["sha256", "integrity", "size", "entries"])
    if (!equalJson(artifact[key], manifest.artifact[key]))
      throw new Error(`Candidate tarball ${key} does not match its manifest.`);
...
async function main() {
  ...
  if (command === "create-manifest") {
    const manifest = await createCandidateManifest({
      ...
      packageJson: JSON.parse(readFileSync(argument("--package-json"), "utf8")),
      packageLock: JSON.parse(readFileSync(argument("--package-lock"), "utf8")),
    });
    writeFileSync(
      argument("--output"),
  ...
  if (command === "verify-artifact") {
    ...
    if (
      manifest.sourceSha !== argument("--source-sha") ||
      manifest.tag !== argument("--tag")
    )
