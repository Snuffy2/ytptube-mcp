...
const subsequentTaskCreate = z
  .object({ ...taskFields, name: taskFields.name.optional() })
  .strict();
...
function result(value: unknown, isError = false) {
  return {
    content: [
      { type: "text" as const, text: JSON.stringify(redact(value), null, 2) },
    ],
    isError,
  };
}
...
export function createServer(
  config: Config,
  client = new YtptubeClient(config),
): McpServer {
      ...
      async (input) => {
        if (mutation && !config.allowMutations) {
          return result(
            {
              error:
                "Mutations are disabled; set YTPTUBE_ALLOW_MUTATIONS=true to enable this tool",
  ...
  const call = (path: string, options?: RequestOptions) =>
  ...
  register(
    ...
    ({ args }) =>
      call("/api/yt-dlp/convert", { method: "POST", body: { args } }),
  );
  register(
    "ytptube_inspect_url",
    ...
    z
      ...
      .strict() as unknown as ZodRawShape,
    (input) =>
      call("/api/yt-dlp/url/info", { query: input as RequestOptions["query"] }),
  );
  register(
    "ytptube_list_logs",
  ...
  register(
    ...
    (input) =>
      call("/api/history/live", { query: input as RequestOptions["query"] }),
  );
  register(
    "ytptube_list_history",
    "List a paginated queue or completed-history page.",
    {
      type: z.enum(["queue", "done"]),
      page,
      per_page: perPage,
    ...
    (input) =>
      call("/api/history", { query: input as RequestOptions["query"] }),
  );
  register(
    "ytptube_get_history_item",
    "Read one queue or history item by ID.",
    { id },
    ({ id }) => call(`/api/history/${pathId(id)}`),
  );
  ...
  register(
    ...
    { id },
    async ({ id }) => {
      const stored = (await call(`/api/history/${pathId(id)}`)) as Record<
        string,
        unknown
      >;
      const allowed = [
  ...
  register(
    ...
    ({ action, ids }) => {
      ...
      return call(
        position ? "/api/history/position" : `/api/history/${action}`,
        {
          method: "POST",
          body: position
            ? { ids: normalizedIds, position: action }
            : { ids: normalizedIds },
        },
      );
    },
  ...
  register(
    ...
    "Delete queue/history records selected by IDs or a status filter; media deletion is opt-in.",
    {
      ...
      delete_media: z.boolean().default(false),
    },
    ({ type, ids, status, delete_media }) => {
      if ((ids === undefined) === (status === undefined))
        throw new Error("Provide exactly one of ids or status");
      return call("/api/history", {
        method: "DELETE",
        body: {
          type,
          ids: Array.isArray(ids)
            ? (ids as Array<string | number>).map(String)
            : undefined,
          status,
    ...
    ({ preset, ids }) =>
      call("/api/archiver", {
        query: {
          ...
          ids: Array.isArray(ids) ? ids.join(",") : undefined,
        },
      }),
  );
  register(
    ...
    ({ id, archived }) =>
      call(`/api/history/${pathId(id)}/archive`, {
  ...
  register(
    ...
    { id: numericId },
    ({ id }) => call(`/api/tasks/${pathId(id)}/metadata`, { method: "POST" }),
    true,
  );
  register(
    ...
    {
      id,
      type: z.enum(["tv", "movie"]).default("tv"),
      overwrite: z.boolean().default(false),
    },
    ({ id, type, overwrite }) =>
      call(`/api/history/${pathId(id)}/nfo`, {
        method: "POST",
        body: { type, overwrite },
      }),
    true,
  ...
  register(
    ...
    (input) => call("/api/tasks", { query: input as RequestOptions["query"] }),
  );
  register(
    "ytptube_get_task",
    "Read a scheduled task by numeric ID.",
    { id: numericId },
    ({ id }) => call(`/api/tasks/${pathId(id)}`),
  );
  register(
  ...
  register(
    ...
    ({ tasks }) => call("/api/tasks", { method: "POST", body: tasks }),
    true,
  );
  register(
    "ytptube_patch_task",
  ...
  register(
    ...
    ({ id, task }) =>
      call(`/api/tasks/${pathId(id)}`, { method: "PUT", body: task }),
    true,
  );
  ...
  register(
    ...
    "List download presets with pagination and sorting.",
    {
      ...
      exclude_defaults: z.boolean().optional(),
    },
    (input) =>
      call("/api/presets", { query: input as RequestOptions["query"] }),
  );
  register(
    "ytptube_get_preset",
    "Read a download preset by numeric ID.",
    { id: numericId },
    ({ id }) => call(`/api/presets/${pathId(id)}`),
  );
  register(
    ...
    { preset },
    ({ preset: input }) =>
      call("/api/presets", { method: "POST", body: input }),
    true,
  );
  register(
  ...
  register(
    ...
    ({ id, preset: input }) =>
      call(`/api/presets/${pathId(id)}`, { method: "PUT", body: input }),
    true,
  );
